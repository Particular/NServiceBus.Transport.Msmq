        ...
        async Task InnerProcessMessages()
        {
            using (var enumerator = inputQueue.GetMessageEnumerator2())
            {
                while (!cancellationTokenSource.IsCancellationRequested)
                {
                    ...
                    void RemoveReceiveTask(Task task, object state)
                    {
                        var receiveTasks = (ConcurrentDictionary<Task, Task>)state;
                        receiveTasks.TryRemove(task, out Task _);
                    }
                    receiveTask.ContinueWith(RemoveReceiveTask, runningReceiveTasks, TaskContinuationOptions.ExecuteSynchronously)
                        .Ignore();
                }
            }
        }
