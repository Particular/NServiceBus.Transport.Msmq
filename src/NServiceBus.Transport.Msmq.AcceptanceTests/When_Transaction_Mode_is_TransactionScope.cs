namespace NServiceBus.Transport.Msmq.AcceptanceTests
{
    ...
    public class When_Transaction_Mode_is_TransactionScope : NServiceBusAcceptanceTest
    {
        ...
        public void Should_throw_if_MSDTC_is_not_active()
        {
            ...
            Assert.That(
                exception.Message, Is.EqualTo("Transaction mode is set to `TransactionScope`. This depends on Microsoft Distributed Transaction Coordinator (MSDTC) which is not available. Either enable MSDTC, enable Outbox, or lower the transaction mode to `SendsAtomicWithReceive`")
                );
        }
